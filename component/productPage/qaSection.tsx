...
export default function QA () {
    ...
    const handleVoteClick = async (QA_id: string) => {
        try {
        ...
        const updatedQADatas = QADatas.map((qa: { QA_id: string; }) =>
  ...
  return (
    <div className={styles.mainContainer}>
        ...
        <div className={styles.qaContainer}>
            {QADatas && QADatas.length > 0 ? (
                QADatas.map((qaData: { 
                    id: Key; question: string; question_date: Date; 
                    answer: string; answer_date: Date; 
                    voted: any; QA_id: string; helpful_count: number | boolean; 
                }) => (
                    <li className={styles.qaList} key={qaData.id}>
                        <p className={styles.qaData}>Q.{qaData.question}</p>
                        <p className={styles.qaDate}>
                            投稿日 {new Date(qaData.question_date)
                                .toISOString()
                                .slice(0, 10)
                                .replace(/-/g, '/')
                        ...
                        <div className={styles.answer}>
                            ...
                            <p className={styles.qaDate}>
                                回答日 {new Date(qaData.answer_date)
                                    .toISOString()
                                    .slice(0, 10)
                                    .replace(/-/g, '/')
                                    .replace(/^(\d{4})\/(\d{2})\/(\d{2})$/, '$1年$2月$3日')
