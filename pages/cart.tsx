...
export default function Cart () {
    ...
    const inCartProducts = addedProducts.filter((product: { status: string; }) => {
        return product.status === "カート";
    })

    const buyLaterProducts = addedProducts.filter((product: { status: string; }) => {
    ...
    const priceforItem = inCartProducts.map((item: { amount: number; price: number; }) => item.amount * item.price);
    const totalProductPrice = priceforItem.reduce((sum: number, price: number) => sum + price, 0);

    const nonBigFurniture = inCartProducts.filter((item: { deliver_category: string; }) => item.deliver_category !== "大型家具");
    const nonBigFurnitureTotalPrice = nonBigFurniture.reduce(
        (sum: number, item: { price: number; amount: number; }) => sum + item.price * item.amount, 0
    );
    ...
    const handleCartItemDelete = async(skuId: string) => {
        try {
    ...
    const makeOrders = async() => {
        try {
            const response = await axios.post('/api/user/cart/make-order', {
                user_email: userData.email,
                productData: inCartProducts,
            }, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
                ...
                <div className={styles.cartContainer}>
                    <div className={styles.productContainer}>
                                ...
                                <div className={styles.otherDetailWrapper}>
                                    <div className={styles.productDetailWrapper}>
                                        ...
                                        {product.selection.includes("/") ? (
                                            product.selection.split("/").map((value: string, index: React.Key) => (
                        ...
                        {buyLaterProducts && buyLaterProducts.length > 0 && (
                            <div className={styles.buyLaterContainer}>
                                ...
                                {buyLaterProducts.map((product: Product) => (
                                    <div className={styles.inCartContainer}>
                                        <div className={styles.productImgWrapper}>
                                            <Image
                                                onClick={() => handleProductPage(product.product_id)}
                    ...
                    {inCartProducts && inCartProducts.length > 0 && (
                        <div className={styles.orderContainer}>
                            ...
                            {inCartProducts && inCartProducts.some((
                                product: { deliver_category: string; }) => product.deliver_category === "大型家具"
                            ) ? (                                
                            ...
                            <div className={styles.btnContainer}>
                                <button className={styles.purchaseBtn} onClick={makeOrders}>
                                    レジへ進む
                                </button>
                                <button className={styles.keepShopBtn} onClick={handleHomePage}>
                                    ショッピングを続ける
                                </button>
                            </div>
                        </div>
