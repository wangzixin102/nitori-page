...
export default function Product ({product, productImg, sku, skuImg, skuProp}: Props) {
  ...
  function mapSkuPropsToNames(sku: { [key: string]: any }[], skuProp: { prop_name: string, prop_id: string }[]) {
  ...
  const selectionProps = skuProp.filter((prop: { select_on_page: number; }) => prop.select_on_page === 0);

  // get selection options in dropdown
  function getSelectionOptions(selectionProps: { prop_name: string; select_on_page: boolean }[], skuWithName: { [key: string]: any }[]): SelectionOption[] {
    ...
    selectionProps.forEach(prop => {
      ...
      selectionOptions.push({
        id: prop.prop_name, // add the id property
  ...
  const skuId = sku.filter((sku: { [x: string]: any; }) => {
    for (let key in sku) {
  ...
  const skuImgs = skuImg.filter((s: { sku_id: string; }) => s.sku_id === skuId)?.map((s: { imgUrl: string; }) => ({ imgUrl: s.imgUrl }));
  const currentImgs = skuImgs.concat([...productImg].map(img => ({ imgUrl: img.imgUrl })));
  
  // filter props show_on_page(info detail)
  const skuShowOnPage = skuProp.filter((prop: { show_on_page: number; }) => prop.show_on_page === 0);
  const propNames = skuShowOnPage.map((prop: { prop_name: string; }) => prop.prop_name);
  const tableSku = filteredSku.map((obj )=> {
    return propNames.reduce((acc: { [x: string]: any; }, prop: string | number) => {
      acc[prop] = obj[prop]
      return acc
    }, {})
  });
  ...
  const formatObject = (obj: { [x: string]: any; }) => {
  ...
  const handleLikedProduct = async (e: { preventDefault: () => void; }) => {
    e.preventDefault();
      try {
    ...
    useEffect(() => {
      async function fetchVoteHistory() {
        ...
        setHasVoted(alreadyLiked.data.some((vote: { sku_id: string; }) => vote.sku_id === currentSku.product_sub_id));
      }
      fetchVoteHistory();
